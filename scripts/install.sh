...
(cd "$ROOT" && pnpm --filter @onenightcarnival/dsh-bridge-browser run build >/dev/null 2>&1)
...
(cd "$ROOT" && pnpm exec dsh plugin --profile web add -w "@onenightcarnival/dsh-bridge-browser@link:$PLUGIN" >/dev/null)
